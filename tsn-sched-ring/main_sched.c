...
//        static_schedule_restart(enum_sched, NULL, NULL);
        ...
        while(rand_prob <= 1)
        {
            printf("***************************TABU RAND PROB: %f**************************\n", rand_prob);
            static_schedule_increment(tabu_sched_out_tradeoff, NULL, NULL, flow_num);
            static_schedule_restart(tabu_sched_out_tradeoff, NULL, NULL, flow_num);
            printf("\n");
            static_schedule_increment(tabu_sched_offset_tradeoff, NULL, NULL, flow_num);
            static_schedule_restart(tabu_sched_offset_tradeoff, NULL, NULL, flow_num);
            printf("\n");
            printf("***************************TABU RAND PROB: %f**************************\n", rand_prob);
            rand_prob += 0.1;
        }
        ...
        while(rand_prob <= 1)
        {
            printf("***************************SA RAND PROB: %f**************************\n", rand_prob);
            static_schedule_increment(sa_sched_out_tradeoff, NULL, NULL, flow_num);
            static_schedule_restart(sa_sched_out_tradeoff, NULL, NULL, flow_num);
            printf("\n");
            static_schedule_increment(sa_sched_offset_tradeoff, NULL, NULL, flow_num);
            static_schedule_restart(sa_sched_offset_tradeoff, NULL, NULL, flow_num);
            printf("\n");
            printf("***************************SA RAND PROB: %f**************************\n", rand_prob);
            rand_prob += 0.1;
        }
